...
import {ActivityIndicator,AppState,Linking,NativeModules,Platform,StyleSheet,View} from 'react-native';
...
function MessengerRoot(){
  const [ready,setReady]=useState(false);const [authed,setAuthed]=useState(false);const [engine,setEngine]=useState<MessengerEngine|null>(null);const [call,setCall]=useState<CallView|null>(null);const [rtConfig,setRtConfig]=useState<any>(null);const [fatal,setFatal]=useState('');const pendingTarget=useRef<Target|null>(null);
  ...
  useEffect(()=>{(async()=>{let url:string|null=null;try{url=await Linking.getInitialURL()}catch{};pendingTarget.current=parseTarget(url||'');if(pendingTarget.current?.token){try{const r=await post('/messenger/handoff/exchange',{token:pendingTarget.current.token});if(r?.token)await setToken(String(r.token))}catch(e:any){setFatal(String(e?.message||e))}}setAuthed(!!(await getToken()));setReady(true)})()},[]);
  useEffect(()=>{const sub=Linking.addEventListener('url',e=>{const target=parseTarget(e.url);if(!target)return;pendingTarget.current=target;if(target.token)post('/messenger/handoff/exchange',{token:target.token}).then(async r=>{if(r?.token){await setToken(String(r.token));setAuthed(true)}}).catch(()=>{});else if(authed&&engine)applyTarget(target,engine).catch(()=>{})});return()=>sub.remove()},[authed,engine]);

  useEffect(()=>{if(!authed)return;let stopped=false;let current:MessengerEngine|null=null;let stopIncoming=()=>{};let stopVoip=()=>{};let received:any;let tapped:any;(async()=>{try{setFatal('');current=new MessengerEngine();await current.boot();if(stopped){current.stop();return}setEngine(current);const cfg=(await get('/realtime/config')).data;setRtConfig(cfg);await callManager.configure(()=>answerIncoming().catch(()=>{}),()=>callManager.end().catch(()=>{}));stopIncoming=callManager.watchIncoming(p=>showIncoming(p).catch(()=>{}),1600);await registerBackgroundNotificationTask().catch(()=>{});registerDevice().catch(()=>{});stopVoip=initializeVoipPush(p=>showIncoming(p).catch(()=>{}));received=Notifications.addNotificationReceivedListener(n=>{const d:any=n.request.content.data||{};if(d.type==='incoming_call')showIncoming(d).catch(()=>{})});tapped=Notifications.addNotificationResponseReceivedListener(r=>{const d:any=r.notification.request.content.data||{};if(d.type==='incoming_call')showIncoming(d).catch(()=>{})});const target=pendingTarget.current;pendingTarget.current=null;if(target)await applyTarget(target,current);}catch(e:any){if(!stopped)setFatal(String(e?.message||e))}})();return()=>{stopped=true;stopIncoming();stopVoip();received?.remove?.();tapped?.remove?.();current?.stop();setEngine(null)}},[authed]);
  ...
  async function showIncoming(payload:any){
    ...
    const mode=(detail?.mode||normalized.mode)==='video'?'video':'voice';const peer=String(detail?.peer?.display_name||normalized.caller_name||'Private Gather member');
    setCall(prev=>prev?.id&&prev.id!==id?prev:{...prev,id,mode,peer,status:'incoming',incoming:true});
    if(AppState.currentState==='active'){
      if(Platform.OS==='android')try{await (NativeModules as any).PrivateGatherCallAccess?.cancelIncomingCallNotification?.(id)}catch{}
      await startMessengerRinger();
    }
  ...
  async function answerIncoming(){if(!call?.id)return;try{await stopMessengerRinger();setCall(prev=>prev?{...prev,status:'answering'}:prev);await callManager.answer();const opened=await callManager.open(call.id,rtConfig,streamState,ended);setCall(prev=>prev?{...prev,mode:opened.detail?.mode||prev.mode,peer:opened.detail?.peer?.display_name||prev.peer,status:'active',incoming:false}:prev)}catch(e:any){setFatal(String(e?.message||e));await callManager.end().catch(()=>{});setCall(null)}}
...
function parseTarget(url:string):Target|null{if(!url||!url.startsWith('privategathermessenger://'))return null;const query=url.split('?')[1]||'';const row:any={};for(const part of query.split('&')){if(!part)continue;const [k,v='']=part.split('=');row[decodeURIComponent(k)]=decodeURIComponent(v.replace(/\+/g,' '))}return {token:row.token||undefined,conversationId:Number(row.conversation_id||0)||undefined,userId:Number(row.user_id||0)||undefined,callMode:row.call_mode==='video'?'video':row.call_mode==='voice'?'voice':undefined}}
